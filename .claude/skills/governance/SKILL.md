...
## Steps
1. Read `.claude/skills/ux-ui-kit/workflows/governance.md` (SemVer table, contribution workflow, deprecation policy, change comms).
...
4. For a **deprecation**: mark with reason + replacement + removal version; keep working ≥ 1 minor cycle; provide a migration map (`.claude/skills/ux-ui-kit/design-systems/crosswalk.md` style); remove only in a major.
...
- `python3 .claude/skills/ux-ui-kit/scripts/validate_tokens.py` passes; contrast re-checked if colors changed.
