...
2. **Verify every state, not just resting.** `node .claude/skills/ux-ui-kit/scripts/verify_states.mjs <file> [--dark]`
   measures default, hover and focus - a button that passes at rest can fail on hover.
3. **One command before reporting done:** `node .claude/skills/ux-ui-kit/scripts/accuracy_report.mjs`.
...
6. **Responsive is gated too:** `node .claude/skills/ux-ui-kit/scripts/verify_responsive.mjs <file|dir>` -
...
> `currentColor`) or plain words. Enforced by `.claude/skills/ux-ui-kit/scripts/check_no_emoji.py`.
...
1. **Token by intent.** Pick the token whose meaning matches the action.
   ...
   bug. Measured by `.claude/skills/ux-ui-kit/scripts/lint_intent.mjs`.
2. **One theme, one source of truth.** Every page renders from the same
   `.claude/skills/ux-ui-kit/tokens/*.json` through one CSS-variable layer imported once at the app root.
