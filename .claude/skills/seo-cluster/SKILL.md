...
instead of WebSearch for SERP data. Run `".claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `".claude/skills/seo/scripts/claude-seo" run render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
