...
1. **Bing Webmaster Tools API**: inbound links, crawl stats, search
   ...
   `".claude/skills/seo/scripts/claude-seo" run bing_webmaster.py` (already shipped with claude-seo).
2. **IndexNow URL submission** for Amazon, Bing, Naver, Seznam.cz,
   Yandex, and Yep via `".claude/skills/seo/scripts/claude-seo" run indexnow_submit.py`.
