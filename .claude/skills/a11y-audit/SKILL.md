...
## Steps
1. Read `.claude/skills/ux-ui-kit/accessibility/wcag-checklist.md` (POUR-organized, P0/P1/P2) and `.claude/skills/ux-ui-kit/accessibility/aria-patterns.md`.
...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (CLAUDE.md → Verification Protocol): `node .claude/skills/ux-ui-kit/scripts/measure_render.mjs <file> [--dark]` (every text element) AND `node .claude/skills/ux-ui-kit/scripts/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 .claude/skills/ux-ui-kit/scripts/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
5. Check reduced-motion handling (`.claude/skills/ux-ui-kit/taste/motion-choreography.md`).
