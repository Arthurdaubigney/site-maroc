...
## Steps
1. Read `.claude/skills/ux-ui-kit/workflows/performance.md` (Core Web Vitals targets, loading strategy, layout-shift, animation perf, design-system runtime cost).
...
4. Kill layout shift: size skeletons to final dimensions, reserve media space via `aspect-ratio` (`.claude/skills/ux-ui-kit/tokens/sizing.json`), never inject content above existing.
5. Animation: only `transform`/`opacity`, `will-change` sparingly, 100–300ms (`.claude/skills/ux-ui-kit/tokens/motion.json`), honor `prefers-reduced-motion`. Prefer CSS state styling over JS for low INP; tree-shake/per-component imports.
