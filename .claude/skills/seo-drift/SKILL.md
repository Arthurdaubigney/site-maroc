...
2. Fetch page via `".claude/skills/seo/scripts/claude-seo" run fetch_page.py <URL>`
3. Parse HTML via `".claude/skills/seo/scripts/claude-seo" run parse_html.py <URL>`
4. Optionally fetch CWV via `".claude/skills/seo/scripts/claude-seo" run pagespeed_check.py <URL>` (use `--skip-cwv` to skip)
5. Hash HTML body and schema content (SHA-256)
6. Store snapshot in SQLite

**Execution:**
```bash
".claude/skills/seo/scripts/claude-seo" run drift_baseline.py <url>
".claude/skills/seo/scripts/claude-seo" run drift_baseline.py <url> --skip-cwv
```
...
**Execution:**
```bash
".claude/skills/seo/scripts/claude-seo" run drift_compare.py <url>
".claude/skills/seo/scripts/claude-seo" run drift_compare.py <url> --baseline-id 5
".claude/skills/seo/scripts/claude-seo" run drift_compare.py <url> --skip-cwv
```
...
```bash
".claude/skills/seo/scripts/claude-seo" run drift_report.py <comparison_json_file> --output drift-report.html
```
...
**Execution:**
```bash
".claude/skills/seo/scripts/claude-seo" run drift_history.py <url>
".claude/skills/seo/scripts/claude-seo" run drift_history.py <url> --limit 10
```
...
- **All URL fetching** goes through `".claude/skills/seo/scripts/claude-seo" run fetch_page.py`, which enforces SSRF protection
