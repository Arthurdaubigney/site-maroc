...
## Steps
1. Read `.claude/skills/ux-ui-kit/design-systems/interop-protocol.md` (Crosswalk Method, the three directions, headless-vs-styled guidance, verification).
2. Use the curated tables in `.claude/skills/ux-ui-kit/design-systems/crosswalk.md` for Material 3 / Apple HIG / Fluent 2 / Carbon / shadcn/ui / Radix. For others, derive a mapping with the Crosswalk Method (map by role/intent across 6 axes: color roles, type scale, spacing unit, radius, elevation, motion).
...
4. **Verify** every mapped color pair for contrast (`.claude/skills/ux-ui-kit/scripts/contrast.py` / `a11y-audit`); confirm all 8 states + dark mode survive the mapping.
