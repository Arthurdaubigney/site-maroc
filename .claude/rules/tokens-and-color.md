...
- `.claude/skills/ux-ui-kit/tokens/colors.json` — 3-tier color system with 6 hues × 11 shades + semantic + component + dark mode
- `.claude/skills/ux-ui-kit/tokens/typography.json` — Major Third (1.25) modular scale + composite text styles
- `.claude/skills/ux-ui-kit/tokens/spacing.json` — 4px base unit scale + semantic spacing aliases
- `.claude/skills/ux-ui-kit/tokens/shadows.json` — 5-level elevation + inner + colored + focus ring
- `.claude/skills/ux-ui-kit/tokens/borders.json` — Radius scale + semantic radii + width scale
- `.claude/skills/ux-ui-kit/tokens/breakpoints.json` — Mobile-first breakpoints + container widths + grid + z-index
- `.claude/skills/ux-ui-kit/tokens/motion.json` — Duration scale + easing curves + transition presets + keyframes + reduced-motion strategy
- `.claude/skills/ux-ui-kit/tokens/gradients.json` — Semantic gradient presets (brand, surface, feedback, accent)
- `.claude/skills/ux-ui-kit/tokens/opacity.json` — Alpha scale (disabled, hover/pressed/selected overlays, scrim)
- `.claude/skills/ux-ui-kit/tokens/blur.json` — Backdrop / frosted-glass blur scale
- `.claude/skills/ux-ui-kit/tokens/sizing.json` — Control size scale + icon sizes + aspect ratios
- `.claude/skills/ux-ui-kit/tokens/states.json` — Semantic interaction-state tokens for the 8 component states
- `.claude/skills/ux-ui-kit/tokens/theming.json` — Multi-brand theme override map + density modes (compact/default/spacious)
- `.claude/skills/ux-ui-kit/tokens/data-viz.json` — Color-blind-aware chart palette (categorical/sequential/diverging) + axis/grid/tooltip tokens
...
- Override map defined in `.claude/skills/ux-ui-kit/tokens/colors.json` → `dark` section
...
5. **Colored shadows** — only on hover states for emphasis (see `.claude/skills/ux-ui-kit/tokens/shadows.json` → `colored`)
...
7. **No emoji, anywhere — not just as icons** (see the ABSOLUTE banner under Verification Protocol). Emoji are inconsistent across platforms and read as machine-generated slop. Never use one as an icon, bullet, status dot, rating face, section marker, or decoration — in UI, code, JSON, copy, comments, or commit messages. Use a real icon set (default: **lucide**) as inline SVG with `currentColor` via the Icon component (`.claude/skills/ux-ui-kit/components/icon-system.md`), or plain words. This includes JS that swaps button labels — swap the `<svg>`/icon, never inject an emoji string. Enforced by `.claude/skills/ux-ui-kit/scripts/check_no_emoji.py` (scans UI + taste + the agent instruction files).
...
1. **One source of truth** — the project's `.claude/skills/ux-ui-kit/tokens/*.json` → a single CSS-variable layer (`:root` + `[data-theme="dark"]`) imported **once** at the app root. Every page references the same semantic tokens; none redefines colors.
2. **No off-theme values** — zero hardcoded hex/px/timing in component/page code. Enforced by `.claude/skills/ux-ui-kit/scripts/lint_hardcodes.py` (the one allowed exception: adapter theme-config that maps our tokens *into* a 3rd-party API, e.g. MUI/Mantine).
3. **Real WCAG, on the source** — the token theme itself passes WCAG 2.2 in **both** light and dark before any page ships. Enforced by `.claude/skills/ux-ui-kit/scripts/validate_contrast.py` (required text/action pairs fail the build; tertiary/decorative are advisory).
