...
2. Use `".claude/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto --json` before HTML/source inspection so SPA content is visible when needed
...
**LCP image subparts** (TTFB, resource load delay, resource load duration, element render delay) are in the CrUX API since the January 2025 release (published February 11, 2025). See `.claude/skills/seo/references/cwv-thresholds.md` for details.

## Tools

```bash
# PageSpeed Insights API (uses header-based API key handling)
".claude/skills/seo/scripts/claude-seo" run pagespeed_check.py URL --json

# SPA-aware HTML/render inspection
".claude/skills/seo/scripts/claude-seo" run render_page.py URL --mode auto --json
...
```bash
".claude/skills/seo/scripts/claude-seo" run pagespeed_check.py URL --json
".claude/skills/seo/scripts/claude-seo" run crux_history.py URL --json
```
