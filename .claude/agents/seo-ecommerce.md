...
2. Analyze product pages with `".claude/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto` and `".claude/skills/seo/scripts/claude-seo" run parse_html.py <URL>`
...
4. If DataForSEO credentials available, fetch marketplace data via
   `".claude/skills/seo/scripts/claude-seo" run dataforseo_merchant.py`

## Cost Guardrails

Before ANY DataForSEO Merchant API call:
```bash
".claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py check <endpoint>
```
...
```bash
".claude/skills/seo/scripts/claude-seo" run dataforseo_costs.py log <endpoint> <actual_cost>
```
