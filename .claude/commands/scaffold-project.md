...
Generate a new design-project skeleton from `.claude/skills/ux-ui-kit/templates/product-design/` (Track B
of `.claude/skills/ux-ui-kit/docs/restructure-plan.md`) into a target directory the user names.
...
<target>/
  ...
  .claude/skills/ux-ui-kit/reference/             real screens Claude studies for context
```
...
2. Copy every file of `.claude/skills/ux-ui-kit/templates/product-design/` into it, preserving the tree.
   ...
   - `.gitkeep` stays in `.claude/skills/`, `.claude/commands/`,
     `src/components/`, `public/images/`, and `.claude/skills/ux-ui-kit/reference/` so the empty folders
...
6. Point `design-tokens.json` at the brand: replace the `primitive.brand` ramp,
   ...
   ```
   python3 .claude/skills/ux-ui-kit/scripts/validate_contrast.py
   ```

7. Remind the user to fill `.claude/skills/ux-ui-kit/reference/` with real screens and `public/images/`
...
```
python3 .claude/skills/ux-ui-kit/scripts/validate_template.py
```
